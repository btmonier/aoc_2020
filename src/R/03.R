...
treeChecker <- function(rightWeight, downWeight, debug = FALSE) {
    i <- 1
    j <- 1
    treeCount <- 0
    rowCount <- 1
    while(TRUE) {
        ...
        if (i > nrow(treeMap)) break
        if (j > ncol(treeMap)) j <- j - ncol(treeMap)
        currPos <- treeMap[i, j]
        if (currPos == "#") treeCount <- treeCount + 1

        if (debug) {
            rowCount <- rowCount + 1
            # Debug / rougelike wannabe ----
            line <- treeMap[rowCount, ]
            line[j] <- "@"
            cat(line, "\n")
            Sys.sleep(0.25)
        }
