...
passwords <- cbind(
    read.table(text = gsub("-", "\t", passwords$V1), sep = "\t"),
    passwords[, 2:3]
)
colnames(passwords) <- c("min", "max", "letter", "password")
passwords$letter <- gsub(":", "", passwords$letter)


## Check function ----
passwordChecker <- function(x) {
    passV <- unlist(strsplit(x[[4]], ""))
    lettLen <- length(passV[passV == x[[3]]])
    if (lettLen >= as.numeric(x[[1]]) & lettLen <= as.numeric(x[[2]])) {
        return(TRUE)
    } else {
        return(FALSE)
    }
...
positionChecker <- function(x) {
    passV <- unlist(strsplit(x[[4]], ""))

    pos1 <- passV[as.numeric(x[[1]])] == x[[3]]
    pos2 <- passV[as.numeric(x[[2]])] == x[[3]]
